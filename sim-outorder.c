...
struct ROB_entry
{
  ...
  INST_TAG_TYPE tag;
  /* ROB slot tag, increment to
               squash operation */
  INST_SEQ_TYPE seq;
  /* instruction sequence, used to
...
static void
/* output stream */
rob_dump(FILE *stream, int context_id)
{
  int num, head;
  struct ROB_entry *rs;
...
static void
/* output stream */
lsq_dump(FILE *stream, int context_id)
{
  int num, head;
  struct ROB_entry *rs;
...
static void
/* output stream */
eventq_dump(FILE *stream, int context_id)
{
  struct RS_link *ev;
...
static void
/* output stream */
readyq_dump(FILE *stream, int context_id)
{
  struct RS_link *link;
...
static void
/* RS to enqueue */
readyq_enqueue(struct ROB_entry *rs)
{
...
static void
/* index of mis-pred branch */
rob_recover(int branch_index, int context_id)
{
...
writeback(void)
{
  ...
  while ((rs = eventq_next_event()))
  {
    ...
    if (contexts[rs->context_id].pred && bpred_spec_update == spec_WB && !rs->in_LSQ && (MD_OP_FLAGS(rs->op) & F_CTRL))
    {
      /* Wattch -- bpred access */
      bpred_access++;
...
lsq_refresh(int context_id)
{
       ...
       i++, index = (index + 1) % LSQ_size)
  {
    ...
    if (/* load? */
        ((MD_OP_FLAGS(contexts[context_id].LSQ[index].op) & (F_MEM | F_LOAD)) == (F_MEM | F_LOAD)) && contexts[context_id].LSQ[index].dispatched && /* queued? */ !contexts[context_id].LSQ[index].queued && /* waiting? */ !contexts[context_id].LSQ[index].issued && /* completed? */ !contexts[context_id].LSQ[index].completed && /* regs ready? */ all_operands_spec_ready(&contexts[context_id].LSQ[index]))
    {
                ...
                int nbytes)       /* number of bytes to access */
{
  ...
  /* check permissions */
  if (!((addr >= mem->ld_text_base && addr < (mem->ld_text_base + mem->ld_text_size) && cmd == Read) || MD_VALID_ADDR(addr)))
  {
    /* no can do, return zero result */
    for (i = 0; i < nbytes; i++)
      ((char *)p)[i] = 0;
...
static void
/* output stream */
mspec_dump(FILE *stream, int context_id)
{
  int i;
...
#define SET_GPR(N, EXPR) (spec_mode                                                                                           \
                              ? ((spec_regs->regs_R[N] = (EXPR)),                                                             \
                                 BITMAP_SET(contexts[bitmap_context_id].use_spec_R, R_BMAP_SZ, (N)) /*spec_regs->regs_R[N]*/) \
                              : (regs->regs_R[N] = (EXPR)))
...
register_rename(void)
{
  ...
  int fetch_redirected[MAX_CONTEXTS];
  /* indicates if fetch has been redirected for each thread
   *  due to a branch for example */
  int fetch_stalled[MAX_CONTEXTS];
  /* indicates if fetch is stalled for each thread, due to
  ...
  struct regs_t *spec_regs;  /* the spec reg set for the thread currently being examined */
                             /* the ID of the context currently being examined */
  static int disp_context_id = 0;
  /* indicates if the current context is in speculative mode */
         ...
         && (include_spec || !contexts[disp_context_id].spec_mode))
  {
        ...
        || (contexts[disp_context_id].fetch_data[contexts[disp_context_id].fetch_head].fetched_cycle + FETCH_RENAME_DELAY > sim_cycle))
    {
      ...
      {
        if ((contexts[j].fetch_num != 0) && (contexts[j].ROB_num < ROB_size) && (contexts[j].LSQ_num < LSQ_size) && (contexts[j].fetch_data[contexts[j].fetch_head].fetched_cycle + FETCH_RENAME_DELAY <= sim_cycle) && (!fetch_stalled[j]))
        {
          disp_context_id = j;
          found = 1;
          break;
    ...
    if (inorder_issue && (last_op.rs && RSLINK_VALID(&last_op) && !all_operands_ready(last_op.rs)))
    {
    ...
    spec_regs->regs_R[MD_REG_ZERO] = 0;
    regs->regs_F.d[MD_REG_ZERO] = 0.0;
    spec_regs->regs_F.d[MD_REG_ZERO] = 0.0;
...
/* Wattch: Get result values */
    ...
    if ((contexts[disp_context_id].pred_PC != regs->regs_NPC && pred_perfect) || ((MD_OP_FLAGS(op) & (F_CTRL | F_DIRJMP)) == (F_CTRL | F_DIRJMP) && target_PC != contexts[disp_context_id].pred_PC && br_pred_taken))
    {
    ...
    if (op != MD_NOP_OP)
    {
      ...
      rs->pred_PC = contexts[disp_context_id].pred_PC;
      rs->in_LSQ = FALSE;
      rs->LSQ_index = -1;
      rs->ea_comp = FALSE;
      ...
      if (MD_OP_FLAGS(op) & F_MEM)
      {
        ...
        lsq->pred_PC = contexts[disp_context_id].pred_PC;
...
void
/* output stream */
fetch_dump(FILE *stream, int context_id)
{
  int num, head;
...
fetch(int *current_fetch_context_ptr)
{
       ...
       i++)
  {
    ...
    if (mem->ld_text_base <= contexts[current_fetch_context].fetch_regs_PC && contexts[current_fetch_context].fetch_regs_PC < (mem->ld_text_base + mem->ld_text_size) && !(contexts[current_fetch_context].fetch_regs_PC & (sizeof(md_inst_t) - 1)))
    {
...
#include "machine.def"
  ...
  for (;;)
  {
    ...
    /* commit entries from ROB/LSQ to architected register file */
    {
      ...
      int i = prev_start;
      while (j < num_contexts)
      {
        commit(i);
        i = (i + 1) % num_contexts;
        j++;
      }
      prev_start = (prev_start + 1) % num_contexts;
    }
